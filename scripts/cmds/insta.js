...
module.exports = {
	config: {
		...
		role: 0,
		shortDescription: {
			vi: "Tải video từ instagram",
			en: "Download video from instagram"
		},
		longDescription: {
			vi: "Tải video/story từ instagram (công khai)",
			en: "Download video/story from instagram (public)"
		},
		category: "media",
		guide: {
			vi: "   {pn} <url video/story>: tải video từ instagram",
	...
	langs: {
		vi: {
			missingUrl: "Vui lòng nhập url video/story instagram (công khai) bạn muốn tải về",
	...
	onStart: async function ({ args, message, getLang, envGlobal }) {
		...
		let msgSend = null;
		try {
			const response = await axios.get(`https://anbusec.xyz/api/downloader/instagram?apikey=${envGlobal.apiKey}&url=${args[0]}`);
