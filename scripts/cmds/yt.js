...
module.exports = {
	config: {
		...
		longDescription: {
			vi: "Tải video/story từ youtube",
			en: "Download video/story from youtube"
		},
		category: "media",
		guide: {
			vi: "   {pn} <url video/story>: tải video từ youtube",
			en: "   {pn} <url video/story>: download video from youtube"
		}
	},

	langs: {
		vi: {
			missingUrl: "Vui lòng nhập url video/shortsyoutube (công khai) bạn muốn tải về",
		...
		en: {
			missingUrl: "Please enter the youtube video/shorts url you want to download",
	...
	onStart: async function ({ args, message, getLang, envGlobal }) {
		...
		let msgSend = null;
		try {
			const response = await axios.get(`https://anbusec.xyz/api/downloader/youtube?apikey=${envGlobal.apiKey}&url=${args[0]}`);
